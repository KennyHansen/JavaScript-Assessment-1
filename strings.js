...
function upperCaseI(leString) {
    var stringArray = leString.split(" ")
    var newString = ""
...
function allLetterA(leString) {
  var countA = 0
  for (var i = 0; i < leString.length; i++) {
    var letter = leString.charAt(i)
    if (letter == 'a' || letter == 'A') countA++;
  }
  console.log(countA)
}

allLetterA(jsHistory)
...
function upperCaseWord(leString, leWord) {
  var stringArray = leString.split(" ")
  var newString = ""
  for (var i = 0; i < stringArray.length; i++) {
      var word = stringArray[i] 
      if (word.includes(leWord)) {
